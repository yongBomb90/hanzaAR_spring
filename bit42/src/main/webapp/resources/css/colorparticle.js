...
// animating circles
    ...
    function tweenCircle(c, dir) {
        ...
        if(dir == 'in') {
            ...
            c.tween = TweenLite.to(c, 0.8, {x: window.innerWidth*Math.random(), y: window.innerHeight*Math.random(), ease:Quad.easeInOut, alpha: 0 , scaleX: 1, scaleY: 1, onComplete: function() {
                c.movement = 'float';
                tweenCircle(c);
            }});
        } else {
            if(c.movement == 'float') {
                c.tween = TweenLite.to(c, 5 + Math.random()*3.5, {x: c.x + -100+Math.random()*200, y: c.y + -100+Math.random()*200, ease:Quad.easeInOut, alpha: 0.0,
                    onComplete: function() {
                        tweenCircle(c);
                    }});
            } else {
    ...
    function formText() {
        ...
        if(textPixels.length < circles.length) {
            for(var j = textPixels.length; j<circles.length; j++) {
                circles[j].tween = TweenLite.to(circles[j], 0.4, {alpha: 0});
            }
        }
    }
...
// event handlers
    ...
    function createText(t) {
        ...
        var pix = ctx.getImageData(0,0,600,1000).data;
    ...
    return {init:init, createText:createText,  explode: explode,domarkertext:domarkertext}
